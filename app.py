...
def index():
    ...
    if request.method == 'POST':
        ...
        lowest_price_towns = data_copy.sort_values('Price').head(5)[['Town', 'Price']]
        lowest_price_towns = []
        for _, row in lowest_price_towns.iterrows():
            town = row['Town']
            price = row['Price']
            if not any(t['Town'] == town for t in lowest_price_towns):
                lowest_price_towns.append({'Town': town, 'Price': price})
                if len(lowest_price_towns) == 5:
                    break
        highest_price_towns = data_copy.sort_values('Price', ascending=False).head(5)[['Town', 'Price']]
        highest_price_towns = []
        for _, row in highest_price_towns.iterrows():
            town = row['Town']
            price = row['Price']
            if not any(t['Town'] == town for t in highest_price_towns):
                highest_price_towns.append({'Town': town, 'Price': price})
                if len(highest_price_towns) == 5:
                    break
